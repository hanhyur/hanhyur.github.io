...
&nbsp;&nbsp;&nbsp;<b>파드(Pod)</b><sup>[1](#footnote_1)</sup>는 쿠버네티스 오브젝트 중 하나로, 하나 이상의 컨테이너 그룹으로 구성되어 있는 쿠버네티스 애플리케이션의 가장 작은 컴퓨팅 단위이다. 파드 안 컨테이너들은 그룹 단위로 스케쥴되어 함께 실행되고 중지되어야 하고, 동일한 물리 머신에서 실행되어야 한다.
...
---
<a name="footnote_1">1</a> : 파드는 고래의 무리를 의미한다고 한다.